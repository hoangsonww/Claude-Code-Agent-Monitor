...
| `DASHBOARD_EVENT_STRING_CAP` | `2048` | Las cargas de los hooks se **recortan antes de guardarse** en `events.data`: se eliminan las copias de archivo completo de las herramientas nativas (`originalFile` en Edit/MultiEdit/NotebookEdit/Write, `file.content` / `file.base64` en Read); se elimina el campo de nivel superior `background_tasks` de cada hook y su tamaño JSON en bytes se registra en `data._trimmed.dropped.background_tasks`; y cada cadena, esté donde esté en la carga, se corta a este número de caracteres (más una breve nota de cuánto se recortó). Cada cambio se anota en `data._trimmed` (mostrado como **Recortado al guardar** en el detalle del evento). Con el recorte activado, lo guardado es una vista previa: la vista de terminal y el diff de cualquier edición por debajo del límite de campo siguen funcionando, el panel "original file" no se muestra para eventos guardados, los resúmenes cuentan líneas de la vista previa y la búsqueda de texto solo ve lo conservado; la transcripción en disco conserva el texto completo. Los eventos de herramientas de subagentes importados desde transcripciones se recortan con las mismas reglas. Ponlo en `0` para desactivar todo el recorte, incluida la eliminación de campos |
