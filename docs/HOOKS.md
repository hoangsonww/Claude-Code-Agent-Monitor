...
- **Working-fleet guard:** the stamp above is skipped when a subagent is still `working` (backgrounded Task agent or Workflow inner agent) — a Stop only ends the *main* turn, and a session whose agents are still running is not idle. It stays **Active** (any stale awaiting flag is cleared) and the `stop` stamp is deferred to the drain check in `SubagentStop`, which fires it when the last working subagent completes
...
- **Drain check:** if the completed subagent was the **last** one working, the main agent is already `waiting` (its turn ended with a `Stop` while agents were still running, so the Waiting stamp was deferred), the session is `active`, and no awaiting flag is set — stamp `awaiting_input_since` (with `awaiting_reason` = `stop`) now. The session flips **Active → Waiting** exactly when the whole fleet is done; a genuine `notification` wait already in place is never downgraded
