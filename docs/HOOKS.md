...
**Purpose:**
- Non-error: set main agent to `waiting` and stamp `awaiting_input_since` (with `awaiting_reason` = `stop`) — Claude finished its turn, ball is in the user's court. The session shows as **Waiting** until `UserPromptSubmit` / `PreToolUse` fires
- **Working-fleet guard:** when a subagent is still `working` (backgrounded Task agent or Workflow inner agent) at Stop time — a Stop only ends the *main* turn, and a session whose agents are still running is not idle — the stamp above is replaced with `awaiting_reason` = `subagent` instead, so the session proactively reads **Waiting** / "SubAgents" (not a plain, unexplained **Active**) for as long as the fleet runs. The `clearAwaitingInputRespectingActor` guard (see PreToolUse/PostToolUse below) preserves this `subagent` stamp against the fleet's own routine tool events; the drain check in `SubagentStop` downgrades it to `stop` once the last working subagent completes
...
- **Drain check:** if the completed subagent was the **last** one working, the main agent is already `waiting` (its turn ended with a `Stop` while agents were still running, so it proactively stamped `awaiting_reason` = `subagent`), the session is `active`, and the awaiting flag is either unset or still `subagent` — stamp `awaiting_input_since` (with `awaiting_reason` = `stop`) now. The session downgrades **"SubAgents" → Waiting/"stop"** exactly when the whole fleet is done; a genuine `notification`/`shell`/`monitor`/`interrupted` wait already in place is never downgraded
...
- If the message matches a permission/input-prompt pattern (`permission`, `waiting for input`, `needs your approval`, `awaiting your response`, …), the server re-derives WHY before stamping — a raw "waiting for input"-shaped message isn't always a genuine ask-the-human block:
  - A **permission/approval-worded** message (`permission`, `approval`) always stamps `awaiting_reason` = `notification` — someone still has to answer it, even if a subagent/shell/monitor call happens to be in flight at the same moment
  - Otherwise, if a subagent is still `working` (`findDeepestWorkingAgent`), stamp `awaiting_reason` = `subagent` instead — main is blocked on its own fleet, not the human
  - Otherwise, if the main agent's `current_tool` is `Bash` (mid a synchronous shell call), stamp `awaiting_reason` = `shell`
  - Otherwise, if `current_tool` is `Monitor` (mid a Monitor tool call), stamp `awaiting_reason` = `monitor`
  - Otherwise, fall back to `awaiting_reason` = `notification` (the genuine case: an idle CLI generically asking for input)

  Either way, `awaiting_input_since` is stamped so the session lands in **Waiting** — the badge label differs per reason (`notification`/`stop`/`session_start`/`interrupted` render as "Waiting" with a suffix chip; `subagent`/`shell`/`monitor` replace the badge word entirely — see `client/src/components/StatusBadge.tsx`)
...
**Working-fleet guard.** Both paths check `findDeepestWorkingAgent` — the same subagent-actor heuristic behind the `Stop` handler's own working-fleet guard (see [§5. Stop](#5-stop) below) — before recovering: spawning a Task/Agent subagent clears the main agent's `current_tool` as soon as `PostToolUse` fires for that tool, which can look identical to a dead interrupted turn if the subagent then runs one long tool call past `DASHBOARD_WORKING_IDLE_SECONDS` with no further hook events. If a subagent is still `working`, neither path recovers the session — it stays **Active**, and `SubagentStop` draining the last subagent (or a later watchdog tick once the fleet finishes) is what resolves its status correctly instead of a false **Waiting**.
