...
product names, Claude Code hook event names (`PreToolUse`, `Stop`, …), tool
names (`Bash`, `Agent`), and the product terms `Agent` / `Subagent` — Spanish is
the deliberate exception, rendering the latter two as `agente` / `subagente`
(asserted in `client/src/i18n/__tests__/i18n.test.ts`). Translate
only the prose around them. In mermaid diagrams, translate labels — never node
IDs.
