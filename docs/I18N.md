...
**Supported languages:** English (`en`), Chinese (`zh`), Vietnamese (`vi`), Korean (`ko`), Spanish (`es`), Italian (`it`)
...
erDiagram
    ...
    LANGUAGE {
        string code "en|zh|vi|ko|es|it"
        string locale "en-US|zh-CN|vi-VN|ko-KR|es-ES|it-IT"
    }
    NAMESPACE {
...
- Keep key parity across `en`, `zh`, `vi`, `ko`, `es`, and `it` files for the same namespace.
...
stateDiagram-v2
    ...
    Detecting --> Loaded_it: localStorage/navigator resolves it
    ...
    Loaded_en --> Loaded_it: user switches to it
    Loaded_it --> Loaded_en: user switches to en
```
...
|---|---|---|
| Resource parity | Same key coverage across `en/zh/vi/ko/es/it` | Missing key detection in CI |
| Locale fallback | Unknown locales fall back to `en` | `es-ES` resolves to `es`, `it-IT` resolves to `it` |
| Terminology consistency | Canonical terms stay stable | `Agent`/`Subagent` expectations |
| Date/number formatting | Locale-specific output shape | `zh-CN`, `vi-VN`, `ko-KR`, `es-ES`, `it-IT` formatting |
...
| Unexpected fallback to English | Unsupported locale code | Ensure code normalizes to `en\|zh\|vi\|ko\|es\|it` and key exists in target namespace |
...
gantt
    ...
    Fill en/zh/vi/ko/es/it namespace files   :a2, after a1, 5d
...
- [ ] Confirm all namespaces exist for `en`, `zh`, `vi`, `ko`, `es`, `it`
- [ ] Confirm key parity across all locale JSON files
- [ ] Confirm language switching works in collapsed and expanded sidebar modes
- [ ] Confirm fallback behavior for region tags (e.g., `es-ES`, `it-IT`, `vi-VN`, `zh-CN`)
