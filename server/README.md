...
| `GET`  | `/api/focus-report/summary`   | Stakeholder-readable **window summary**: 2–4 plain-language LLM-synthesized bullets for the same window/scope (`lib/focus-summary.js`, cached in `focus_summaries` gated by an input digest). `{ summary: null }` (200) when the LLM path is off/unavailable, the window is empty, or generation failed. See below |
