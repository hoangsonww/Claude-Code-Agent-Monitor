...
describe("error-run capture (no crash)", () => {
  ...
  it("degrades a hand-stored out-of-range window_days to an error run (not a throw)", () => {
    ...
    // error-status run rather than throwing a RangeError. The value must push
    // (now - window_days days) past the ±8.64e15 ms Date range for ANY current
    // date — 999999999 days ≈ 8.64e16 ms, an order of magnitude past the limit,
    // so the RangeError fires regardless of when the test runs.
    db.prepare(
      `INSERT INTO report_definitions (id, name, template, frequency, hour, tz_offset, formats, enabled, next_run_at, window_days)
       VALUES ('rs-bigwin', 'big', 'session_throughput', 'daily', 9, 0, '["json"]', 1, ?, 999999999)`
    ).run("2026-01-01T09:00:00.000Z");
    const { runReportForDefinition } = require("../routes/reports");
