...
function queryAgents(filters, limit, offset) {
  ...
  return {
    rows,
    total,
    columns: ["id", "session_id", "status", "type", "started_at", "ended_at", "event_count"],
  };
}
...
function queryEvents(filters, limit, offset) {
  ...
  return {
    rows,
    total,
    columns: ["id", "session_id", "agent_id", "event_type", "tool_name", "summary", "created_at"],
  };
}
...
router.get("/export", (req, res) => {
  ...
  if (format === "json") {
    ...
    return res.json({
      ...
      truncated,
      message: truncated
        ? `Export limited to ${EXPORT_LIMIT} rows; ${result.total} total rows match.`
        : undefined,
    });
  }
