...
사이드바에서 대시보드, 포커스, Kanban 보드, 세션 목록, 활동 피드, 분석, 워크플로, 설정에 빠르게 접근할 수 있습니다. 각 페이지는 실시간 업데이트와 풍부한 시각화로 Claude Code 에이전트 활동에 대한 깊이 있는 인사이트를 제공하도록 설계되었습니다.
...
| **포커스**(활동 리포트) | `/focus`, 사이드바 레이블 **포커스**, 달력 바로 다음에 위치하는 독립 페이지로, 스윔레인 그리드 대신 평범하고 이해관계자가 읽기 쉬운 목록으로 "우리가 실제로 무엇을 했는가"에 답합니다. 달력 보드와 동일한 세 가지 독립적인 필터 — 프로젝트 칩("모든 프로젝트"와 호박색 **"미할당"** 칩 포함), 전역 세션 드롭다운, 그리고 하루/사용자 지정 범위 기간 선택기 — 를 그대로 공유하며, 정확히 동일한 `ProjectScopeFilters` 컴포넌트와 `GET /api/focus-report` 엔드포인트를 재사용하므로 백엔드 작업이 전혀 필요하지 않았습니다. 동일한 통계 타일(총 에이전트 시간, Concurrency, 선언된 항목 비율 %, 계획 외 비율 %, 유휴 제외) 아래에는 새로운 **활동 카드**가 있습니다: 선택한 구간에서 일어난 서로 다른 일마다 한 행씩 — 플랜 항목, 우회 작업/버그/기능(그것에 걸린 모든 세션에 걸쳐 제목별로 그룹화됨), 또는 미분류 시간 — 각 행은 종류 칩(앱에 이미 있는 `FOCUS_KIND_CONFIG`/`FOCUS_KIND_ICONS` 어휘를 재사용), 실제 경과/활성 시간 수치를 표시하며, 분류기가 **추론한** 항목의 경우 한 문장짜리 `inferred_reason`도 함께 표시합니다(`client/src/lib/focusActivity.ts`의 `groupFocusActivity()`가 cwd별로 키를 부여하므로 서로 다른 두 프로젝트의 동일한 항목 번호가 절대 합쳐지지 않습니다; 하나의 키에 두 개 이상의 세그먼트가 겹치면 표시되는 레이블/이유는 시간 비중이 가장 큰 세그먼트에서 가져오며 "세션 N개 더" 메모가 붙습니다). "모든 프로젝트" 범위에서는 각 행 앞에 해당 프로젝트 이름이 붙고, 단일 프로젝트 범위에서는 이 접두어가 생략됩니다. 5행을 넘는 항목은 "더 보기"/"간략히 보기" 토글 뒤로 접힙니다. 실시간 `ccam focus push/bug/feature` 선언은 오늘 기준으로 자신의 레이블과 별개인 이유를 갖지 않습니다 — 분류기가 추론한 항목만 이유를 갖습니다 — 아직 해결되지 않은 알려진 공백입니다 |
| **Kanban 보드**                   | 헤더 토글(`localStorage`에 저장)로 전환하는 세 가지 보기: **에이전트** — 4개 컬럼(Working / Waiting / Completed / Error); **세션** — 5개 컬럼(Active / Waiting / Completed / Error / Abandoned); 그리고 **프로젝트** — 프로젝트별로 하나씩 컬럼이 생성되며(세션은 해당 프로젝트에 매핑된 폴더 기준으로 그룹화되고, 상태 컬럼과 구분되도록 컬럼 색상이 순환됩니다), 아직 어떤 프로젝트에도 속하지 않은 폴더의 세션을 위한 **미할당** 컬럼이 마지막에 추가됩니다. 적어도 하나의 **모니터** 그룹을 만들면(물리적 디스플레이를 본뜬 것으로, 헤더의 **+ 모니터 추가** 버튼으로 생성) 거기에 배정된 프로젝트 컬럼은 그 모니터 전용의 테두리 있는 드래그 가능한 박스 안에 실제로 담겨 렌더링됩니다 — 단순히 태그만 붙는 게 아니라 진짜로 그 안에 포함됩니다 — 이 박스들은 같은 가로 스크롤 한 줄 안에서 서로 나란히 놓이고, 마지막에는 아직 배정되지 않은 프로젝트 컬럼을 실제로 담는 **미분류** 박스가 추가됩니다(모니터 박스와 같은 컨테이너지만 고정되어 있어 드래그·이름 변경·삭제가 불가능합니다). 프로젝트 컬럼을 어떤 모니터의 박스 위로(또는 이미 그 박스 안에 있는 다른 컬럼 위로) 드래그하면 그 박스 안으로 옮겨지고, 미분류 박스 위로(또는 행 끝을 지나서) 드래그하면 배정이 해제되어 그 컬럼이 미분류 박스 안에 들어가며, 한 모니터의 박스를(그 헤더를 통해) 다른 모니터의 박스 위로 드래그하면 박스 전체가 좌우로 위치를 바꿉니다 — 실제 책상 위에서 모니터가 놓인 위치를 그대로 반영합니다. 박스 이름은 그 자리에서 바로 수정할 수 있고, 헤더의 셰브론을 통해 접거나 펼칠 수 있으며 — 접으면 박스 전체가 메인 행에서 완전히 빠져나와 그 위의, 접힌 모든 모니터가 공유하는 얇은 띠로 옮겨가므로 그저 좁아진 채로 자리만 차지하는 대신 실제로 가로 공간을 확보하고, 펼치면 원래 순서대로 메인 행으로 다시 내려옵니다(그동안 컬럼은 계속 마운트된 채로 시각적으로만 숨겨지므로 카드 자체의 "더 보기" 상태가 토글 후에도 유지됩니다) — 삭제하면 그 안의 프로젝트는 미분류로 돌아갑니다. 모든 프로젝트 컬럼(그리고 독립적인 미할당 컬럼)에도 동일한 셰브론이 있어, 그 컬럼이 어느 박스 안에 있든 상관없이 해당 컬럼 하나만 제자리에서 얇은 제목 표시줄만 남도록 접을 수 있습니다. 이 두 가지 모두 순전히 `localStorage`(`kanban-monitors` / `kanban-monitor-map` — 후자는 각 모니터의 `collapsed` 플래그도 함께 담습니다. 컬럼별 접힘 상태는 `kanban-collapsed-projects`에 담깁니다)에 저장되는 개인 설정이며 서버로 전송되지 않습니다. **Waiting** 컬럼은 에이전트에 저장된 `waiting` 상태에 직접 매핑됩니다 — Claude Code가 프롬프트에서 대기 중일 때(새 세션, 턴 사이, 또는 권한 Notification으로 차단된 상태) 설정되며, 사용자가 재개하는 순간(UserPromptSubmit / PreToolUse) `working`으로 전환됩니다. 각 컬럼 헤더에는 라이프사이클 전환을 설명하는 `?` 툴팁이 표시됩니다. 카드는 저장된 상태별로 서버에서 가져오고(상태당 사실상 무제한), 이후 클라이언트 측에서 컬럼당 10개 카드로 페이지네이션되며 "더 보기" 버튼이 제공됩니다. WS 구독은 활성 보기에 범위가 지정되므로(`agent_*` vs `session_*` 프레임) 보이지 않는 보기의 업데이트가 다시 가져오기를 유발하지 않습니다. Waiting 배지는 행의 `awaiting_reason`을 호버 툴팁으로 표시합니다 — **입력 필요** (`notification`), **턴 완료** (`stop`), **프롬프트 대기** (`session_start`), **중단됨** (`interrupted`) — 컴팩트한 카드에서는 제목 공간을 유지하기 위해 툴팁으로만 표시되며, 더 넓은 화면(세션 테이블, 세션 상세 헤더)에서는 추가로 이유를 중첩된 칩 형태로 인라인 표시하고, 긴급한 이유(권한 프롬프트, 중단)는 더 진한 앰버 색으로 렌더링됩니다. 세 가지 이유가 더 있는데 — **SubAgents** (`subagent`), **Shell** (`shell`), **Monitor** (`monitor`) — 이는 Claude가 사람에게 막혀 있는 것이 아니라 자식(자신이 생성한 서브에이전트 플릿, 동기적인 Bash 호출, 또는 Monitor 도구 호출)을 통해 여전히 활발히 작업 중임을 의미하므로, "Waiting" 안에 중첩되는 대신 컴팩트한 카드에서도 배지의 단어 자체를 완전히 대체합니다. 각 카드의 모델 칩은 세션이 프리미엄/고비용 모델(Opus 또는 Fable 계열)을 실행 중일 때 밝은 빨간색으로 렌더링됩니다 |
