...
window.__WIKI_CONTENT_I18N = {
  zh: {
    ...
    "For a live monitoring session, <code>ccam repl</code> (aliases <code>shell</code> / <code>i</code>) opens an interactive shell where you type commands without the <code>ccam</code> prefix — with tab-completion, persisted arrow-key history, and a live server-status prompt (<code>●</code> up / <code>○</code> offline). Each entered line runs as an isolated child process, so an offline refusal or a blocking <code>tail</code> never takes the shell down.":
      "为了持续监控，<code>ccam repl</code>（别名 <code>shell</code> / <code>i</code>）会打开一个交互式 shell，你可以在其中输入命令而无需 <code>ccam</code> 前缀——支持 Tab 补全、可持久化的方向键历史，以及实时服务器状态提示符（<code>●</code> 在线 / <code>○</code> 离线）。每一行都作为独立子进程运行，因此离线拒绝或阻塞的 <code>tail</code> 都不会拖垮 shell。",
    Group: "分组",
    Commands: "命令",
    "What you get": "提供的能力",
    Monitoring: "监控",
  ...
  ko: {
    ...
    "For a live monitoring session, <code>ccam repl</code> (aliases <code>shell</code> / <code>i</code>) opens an interactive shell where you type commands without the <code>ccam</code> prefix — with tab-completion, persisted arrow-key history, and a live server-status prompt (<code>●</code> up / <code>○</code> offline). Each entered line runs as an isolated child process, so an offline refusal or a blocking <code>tail</code> never takes the shell down.":
      "지속적인 모니터링을 위해 <code>ccam repl</code>(별칭 <code>shell</code> / <code>i</code>)은 <code>ccam</code> 접두사 없이 명령어를 입력하는 대화형 셸을 엽니다 — Tab 자동 완성, 저장되는 방향키 히스토리, 그리고 실시간 서버 상태 프롬프트(<code>●</code> 실행 중 / <code>○</code> 중지)를 제공합니다. 입력된 각 줄은 격리된 자식 프로세스로 실행되므로 오프라인 거부나 블로킹되는 <code>tail</code>이 셸을 종료시키는 일은 없습니다.",
    Group: "그룹",
    Commands: "명령",
    "What you get": "제공 기능",
    Monitoring: "모니터링",
