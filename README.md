...
ccam focus pop                    # resolve the current detour
...
| `Focus`        | `ccam focus …` run in the session's Bash tool | Parsed out of the `PostToolUse` hook's `tool_input.command` (the only channel that natively carries the session id). `set`/`push`/`pop`/`done` update the session's `session_focus` row (plan-item pointer + detour stack) and write a `Focus` event with an item-text snapshot for the timeline; broadcasts `session_focus` (and `plan_updated` after `done`). `status` is read-only and records nothing |
