...
The sidebar provides quick access to the Dashboard, Projects, Calendar, Kanban Board, Sessions list, Activity Feed, Analytics, Workflows, and Settings. Each page is designed to give you deep insights into your Claude Code agent activity with real-time updates and rich visualizations.
...
| **Calendar** (Focus Calendar board) | Standalone page (`/focus-calendar`, sidebar label **Calendar**, positioned right after Projects) rendering the same swimlane day-view calendar as the per-project focus-time report's Calendar tab, but across **every** monitored project at once. Three independent filters compose by intersection rather than a dependency chain: a **project** filter (optional, default "all projects"), a **session** filter (a single global dropdown listing every session across every project — not scoped to whichever project is currently selected, so picking a session whose folder isn't part of the selected project is a legitimate combination that simply yields an empty result), and a **time-period** filter (day-by-day Prev/Today/Next navigation, defaulting to "today," plus a custom date-range mode) — changing any one never resets another. Powered by a new aggregate endpoint, `GET /api/focus-report`, which always requires an explicit `from`/`to` window (no hidden server-side default) and reuses `server/lib/focus-report.js`'s report-building functions completely unchanged. The existing per-project report modal (opened from a project's card) is unaffected — its stat-tile/List-Calendar-toggle/list-body rendering is shared with this page via `FocusReportBody.tsx` rather than duplicated. Because a report can now span multiple projects, the Concurrency stat tile is relabeled **"Concurrent agent sessions"** here (the same `concurrency_ratio` figure, read correctly as cross-project overlap instead of one project's own multitasking) |
