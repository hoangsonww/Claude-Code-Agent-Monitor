...
| `DASHBOARD_EVENT_STRING_CAP` | `2048` | Hook payloads are **trimmed before they are stored** as `events.data`: the native tools' whole-file mirrors (`originalFile` on Edit/MultiEdit/NotebookEdit/Write, `file.content` / `file.base64` on Read) are dropped; the top-level `background_tasks` field is dropped from every hook, with its JSON byte size recorded at `data._trimmed.dropped.background_tasks`; and every string anywhere in the payload is cut to this many characters (plus a short note of how much was cut). Every change is noted under `data._trimmed` (shown as **Trimmed on storage** in the event detail pane). With trimming on, what is stored is a preview: the terminal view and the diff of any edit under the field cap keep working, the "original file" pane is not shown for stored events, summaries count preview lines, and text search over events sees only what was kept — the transcript on disk keeps the full text. Without this, PostToolUse rows grow the database by hundreds of MB a week. Subagent tool events imported from transcripts are trimmed by the same rules. Set to `0` to disable all trimming, including field drops |
