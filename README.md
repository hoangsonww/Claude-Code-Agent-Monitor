...
| **Session Detail**                 | Per-session agent hierarchy tree (parent/child) and full event timeline; click any event to expand and see its full hook payload (tool_input, tool_response, etc.) rendered as parsed fields                                                                                |
| **Activity Feed**                  | Real-time streaming event log with pause/resume and pagination; click any event row to expand its full hook payload inline; a dedicated "Session →" button at the right of each row navigates directly to session details without collapsing the feed                        |
