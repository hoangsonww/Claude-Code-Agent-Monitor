...
sequenceDiagram
    ...
    Note over TX: SessionStart → create/reactivate session,<br/>stamp awaiting_input_since (fresh CLI = Waiting),<br/>abandon stale sessions (DASHBOARD_STALE_MINUTES,<br/>default 180)<br/>UserPromptSubmit → clear awaiting flag,<br/>main agent → working<br/>PreToolUse → clear awaiting flag, agent working<br/>PostToolUse → clear awaiting flag, clear current_tool<br/>Stop → main agent idle, awaiting flag re-stamped<br/>(non-error); error stop drops the flag<br/>SubagentStop → mark subagent done<br/>(does NOT touch awaiting flag)<br/>Notification → permission/input-prompt pattern<br/>stamps awaiting_input_since on session + main agent<br/>SessionEnd → drop awaiting flag, mark all completed<br/>Every event → detect compaction from JSONL,<br/>create Compaction agent + event if new
