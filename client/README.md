...
│   │   ├── StatTile.tsx        # Single labeled stat cell — extracted out of FocusReportBody so FocusPage reuses it unchanged
...
│   │   ├── focusActivity.ts # groupFocusActivity() — per-cwd rollup of a FocusReport's segments into one row per plan item/detour-bug-feature/unclassified bucket, for FocusPage
...
**`groupFocusActivity(report, projectLabelForCwd?)`** (`lib/focusActivity.ts`) — walks every session's segments in an already-window-clipped `FocusReport` (the aggregate `GET /api/focus-report` endpoint clips server-side, so no client-side windowing like `windowedTotals.ts` is needed here) and groups them by `${cwd}:item:${item_number}` / `${cwd}:${kind}:${label}` / `${cwd}:none`, summing wall/active/idle time. When more than one segment lands on the same key, the displayed label/`inferred`/reason come from whichever contributed the largest `wall_ms` share; `contributions` records how many segments rolled in. Sorted by `wallMs` descending.
