...
stateDiagram-v2
    ...
    active --> waiting: Stop, không lỗi, không subagent nào đang working (cờ được đóng dấu lại)
    active --> active: Stop khi một subagent còn working (Đang chờ bị hoãn)
    active --> waiting: SubagentStop cuối cùng hoàn tất (đóng dấu stop đã hoãn)
...
ccam focus pop                    # kết thúc detour hiện tại
...
| `Focus`        | Chạy `ccam focus …` trong tool Bash của phiên | Được parse từ `tool_input.command` của hook `PostToolUse` (kênh duy nhất mang sẵn session id). `set`/`push`/`pop`/`done` cập nhật hàng `session_focus` của phiên (con trỏ mục kế hoạch + ngăn xếp detour) và ghi một sự kiện `Focus` kèm ảnh chụp nội dung mục cho dòng thời gian; phát `session_focus` (và `plan_updated` sau `done`). `status` là chỉ-đọc và không ghi nhận gì |
