...
**语言支持 / Language Support**: English (`en`) · 中文 (`zh`) · 越南语 (`vi`) · 韩语 (`ko`) · Español (`es`) · Italiano (`it`)

切换文档：[`README.md`](./README.md) · [`README-CN.md`](./README-CN.md) · [`README-VN.md`](./README-VN.md) · [`README-KO.md`](./README-KO.md) · [`README-ES.md`](./README-ES.md) · [`README-IT.md`](./README-IT.md)
...
```mermaid
flowchart LR
    A["用户选择语言<br/>en / zh / vi / ko / es / it"] --> B["前端 i18n 路由"]
    B --> C["翻译字典加载"]
    C --> D["UI 文案与可访问性标签渲染"]
...
| **界面本地化** | 内置语言切换，UI 文案与无障碍标签已覆盖英文（`en`）、中文（`zh`）、越南语（`vi`）、韩语（`ko`）、西班牙语（`es`）和意大利语（`it`）。覆盖范围现已贯穿 Workflows 页面的所有 tooltip：统计卡片的计算说明与按值分桶的解读、每个图表的「此图展示什么 / 如何阅读 / 为何重要」浮层、所有图形悬停 tooltip（编排 DAG、工具流、Pipeline、模型委派、并发时间线）、Workflow Patterns 详情面板的叙述与建议、设置页 → 模型定价的信息浮层、CLAUDE_HOME 面板，以及完整的 Import History 流程 |
| **种子数据** | 内置种子脚本，用于演示和开发 |
...
| **启动画面** | 应用加载时每个浏览器会话显示一次的品牌开场画面:根据时间的问候语（早上好 / 下午好 / 晚上好 / 夜深了）、一句醒目的本地化标语与两行副文案,以及深色背景（径向光晕、星座连线、颗粒质感）上带动画的节点图品牌标记。从首帧起即**不透明**（应用内容不会闪现）,停留约 2.5 秒后淡出,点击任意处可跳过,尊重 `prefers-reduced-motion`,已本地化 en/zh/vi/ko/es/it |
...
**设置 → 声音**提供总开关、音量滑块，以及每种提示音的独立开关。打开某个开关会立即播放该提示音，让你清楚听到刚刚启用的是什么；**试听**按钮可随时播放完成提示音。偏好设置保存在 `localStorage` 的 `agent-monitor-sound` 键下，在整个应用内即时生效——无需重新加载。该面板支持英文、中文、越南语、韩语、西班牙语和意大利语。
